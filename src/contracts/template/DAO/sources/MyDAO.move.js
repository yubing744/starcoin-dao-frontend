const MyDAOSourceTpl = (
  ...
  for (const k in members) {
    ...
    const t = count > 0 ? '\t\t' : '';
    const n = count != plugins.length - 1 ? '\n' : '';
    membersInfo += `${t}DAOSpace::member_offer(&dao_root_cap, ${value}, Option::none<vector<u8>>(), Option::none<vector<u8>>(), 1);${n} `;
    count++;
  }
...
// DAOSpace::join_member_with_root_cap(&dao_root_cap, @${member}, Option::none<vector<u8>>(), Option::none<vector<u8>>(), 1000);
